...
router.get("/", (req, res) => {
  Students.getStudents(req.token.id)
    .then((students) => {
      res.status(200).json({ message: "Rendering student list: ", students });
    })
    .catch((err) => {
      res.status(500).json({
        errorMessage:
  ...
  Students.findStudent(id)
    .then((student) => {
      if (!student) {
        res
          .status(404)
          .json({ errorMessage: "Could not find student with set id." });
      } else {
        res
          .status(200)
          .json({ message: "Pulling up student information: ", student });
      }
    })
    .catch((err) => {
      res.status(500).json({
        errorMessage:
          "Server error, could not find student id. Contact backend.",
        err,
      });
    });
});
  ...
  Students.findStudent(id)
    .then((student) => {
      if (!student) {
        res
          .status(404)
          .json({ errorMessage: "Unable to edit student; set id not found." });
      } else {
    ...
    .catch((err) => {
      res
        .status(500)
        .json({ errorMessage: "Server errror, unable to update student", err });
    });
});
